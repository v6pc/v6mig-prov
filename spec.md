...
日本国内において、NTT 東西によるフレッツ光ネクストの NGN IPv6 契約者数は、1,200 万契約を超過しており、NGN IPv6 普及率も 57.8% となっており、IPv6 サービスの本格的な普及が進んでいる。(2018年12月時点)　
...
本文書は、上記問題を解決することを目的として、国内標準のプロビジョニング方式の仕様を策定したものであり、特に新規 VNE 事業者に対して当該標準方式の採用を期待するものである。尚、既にサービス提供中の VNE事 業者においては、国内標準方式へ移行する上での得失を考慮した上で判断いただければ幸いである。
...
### 1.2. 用語集
 ...
 - プロビジョニング方式対応 DNS サーバ
   - CPE が参照するフルサービスリゾルバであり、本文書で規定する専用ドメイン名に対する TXT レコードの問い合わせに対し、本文書で規定するフォーマットで記述された TXT レコードを返す DNS サーバ
...
- a) 開始
- b) DNS クエリーで AAAA, TXT リソースレコードの情報を取得する
- c) プロビジョニングサーバのアドレスと通信方式が取得できたか
  - c.1) 取得に成功し、初回取得の場合もしくは前回と同じ TXT が得られた場合
    - d) へ
  - c.2) 取得に成功したが、前回と異なる TXT が得られた場合
    - CPE にマイグレ技術が動作している場合、機能を無効にする
    - d) へ
  - c.3) NODATA もしくは NXDOMAIN で取得できなかった場合
    - CPE にマイグレ技術が動作している場合、機能を無効にする
    - 1-3 時間のランダムな時間を待つ
    - a) へ
  - c.4) NODATA もしくは NXDOAIN 以外の理由で取得できなかった場合
    - 1-10分のランダムな時間を待つ
    - a) へ
- d) プロビジョニングサーバへ情報へ CPE 情報を送信する
...
- i) 以前にマイグレ技術が設定されて有効か
  ...
  - i.2) 無効な場合
    - k) へ
- k) プロビジョニングデータに合わせて、CPE を設定する
- l) プロビジョニングデータに TTL が設定されているか
  - l.1) 設定されている場合
    - TTL の時間待つ
    - a) へ
  - l.2) 設定されていない場合
